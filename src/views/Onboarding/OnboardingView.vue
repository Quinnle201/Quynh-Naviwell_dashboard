...
export default {
    ...
    methods: {
        ...
        getFamilyHistory() {
            axiosInstance.get('/family-history')
                .then(response => {
                    this.famHx = response.data.map(i => ({ 'label': i.name, 'value': i.id }));
                })
                .catch(error => {
                    console.log(error)
        ...
        getMedConditions() {
            axiosInstance.get('/medical-conditions')
                .then(response => {
                    this.medConditions = response.data.map(i => ({ 'label': i.name, 'value': i.id }));
                })
                .catch(error => {
                    console.log(error)
                    this.alertStore.error(error.response.data.message)
        ...
        onSubmit(values) {
            ...
            axiosInstance.put(`/patients/${this.patientId}`, values)
                .then(async response => {
                    try{
                        //sync famHx and medConditions
                        await axiosInstance.post(`/patients/${this.patientId}/history`, {id: this.selectedFamHx.map(item => item.value) })
                        await axiosInstance.post(`/patients/${this.patientId}/condition`, {id: this.selectedMedConditions.map(item => item.value) })
                    } catch(error) {
                        this.alertStore.error("Syncing profile data failed. Please report to administrator")
                        console.log(error);
                    };
        ...
        <div class="welcome-inner">
            ...
            <Form class="info-form" @submit="onSubmit" :initial-values="ptData">
                <div v-show="screen == 0" class="info-form-inner">
                    <div class="info-form-inner-item">
                        ...
                        <label class="info-form-item" >
                            Gender
                            <Field as="select" name="profile.gender" :class="genderError ? 'info-form-item-error' : ''" @change="genderError = false">
                                <option value="" disabled>Select your gender</option>
                                <option v-for="gender in genderValues" :value="gender.value">{{ gender.label }}</option>
                            </Field>
                        </label>
                        <label class="info-form-item">
                            Ethnicity
                            <Field placeholder="Enter your ethnicity group" name="profile.ethnicity"></Field>
                        </label>
                        <label class="info-form-item">
                            Spoken Languages
                            <Field placeholder="Enter languages that you speak" name="profile.language"></Field>
                        </label>
                    </div>
                ...
                <div v-show="screen == 1" class="info-form-inner">
                    <div class="info-form-inner-item">
                        <label class="info-form-item" :class="phoneError ? 'info-form-item-error' : ''">
                            Phone number
                            <Field placeholder="+1 (XXX) XXX-XXXX" type="tel" name="user.phone" @change="phoneError = false" />
                        </label>
                        <label class="info-form-item">
                            Street address
                            <Field placeholder="Street address" type="text" name="contact.address" />
                        </label>
                        ...
                        <div class="info-form-add-btn" @click="toggleEmergency()">
                            ...
                            {{ emergencyContactShown ? "Remove emergency contact info" : "Add emergency contact info" }}
                        </div>
                        <Transition>
                ...
                <div v-show="screen == 2" class="info-form-inner">
                    <div class="info-form-inner-item">
                        <label class="info-form-item">
                            Family history
                            <VueMultiselect v-model="selectedFamHx" :options="famHx" :multiple="true"
                                :close-on-select="false" track-by="value" label="label" search="false"
                                placeholder="Family history" select-label="Select" :allow-empty="false"
                                deselect-label="Remove">
                            </VueMultiselect>
                        </label>
                        <label class="info-form-item">
                            Health conditions
                            <VueMultiselect v-model="selectedMedConditions" :options="medConditions" :multiple="true"
                                :close-on-select="false" track-by="value" label="label" search="false"
                                placeholder="Health conditions" select-label="Select" :allow-empty="false"
                                deselect-label="Remove">
                            </VueMultiselect>
                        </label>
                        <label class="info-form-item">
