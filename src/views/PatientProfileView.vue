...
export default {
  ...
  methods: {
    ...
    getPatient() {
      axiosInstance
        .get(`/patients/${this.user.profile_id}`)
        .then((response) => {
    ...
    getPatientHealthData() {
      axiosInstance
        .get(`/patients/${this.user.profile_id}/health-data`)
        .then((response) => {
    ...
    setPatientData(patient) {
      ...
      this.generalInfo.fields.forEach((item) => {
        if (item.model) {
          this.$refs.populatedForm.setFieldValue(
            item.name,
            _get(pt, item.model)
          );
        }
    ...
    onSubmit(values) {
      ...
      axiosInstance
        .put(`/patients/${this.user.profile_id}`, values)
        .then((response) => {
            ...
            <div class="patient-data light-bg patient-card pt-card">
              ...
              <ul>
                ...
                <li>
                  Resting HR
                  <span>{{
                    patient.current_health_data?.resting_hr ?? "-"
                  }}</span>
                </li>
              </ul>
                    ...
                    <div class="info-form-item-wrapper">
                      <label class="info-form-item">
                        ...
                        >
                          ...
                          <option v-for="code in medicine" :value="code.value">
                            {{ code.name }}
                          </option>
                        </Field>
                      </label>
